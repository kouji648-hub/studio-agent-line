...
#### 並列実行可能なケース
**例**: 新規撮影メニュー追加（マタニティフォト）
- メイン（Sonnet）: pattern-b-timing.html にマタニティ選択肢追加
- サブ1（Sonnet）: assets/images/maternity/ 画像最適化
- サブ2（Sonnet）: Playwrightテスト追加

**実行方法**: tmuxで3ペイン分割、各ペインで独立して作業

#### 直列実行必須のケース
**例**: 料金体系変更
- 全料金カードのsubtitle更新 → テスト → コミット → デプロイ
- 1つのエージェントで順次実行（整合性担保）

### コスト最適化目標
- Sonnet使用率: 90%
- Opus使用率: 10%
- 月間推定コスト: $3-5（このプロジェクト単体）
